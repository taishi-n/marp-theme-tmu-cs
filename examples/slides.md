...
<audio class="wavesurfer-spectrogram" controls src="../assets/sine-stereo-440-660.wav" data-spectrogram-height="120" data-spectrogram-fft-samples="2048"></audio>

<!--
このページでは wavesurfer.js と Spectrogram plugin を使った可視化例を説明します。
`audio.wavesurfer-spectrogram` を書くと，エンジンが audio 要素を波形・スペクトログラム付きの UI に wrap します。
`data-spectrogram-height` と `data-spectrogram-fft-samples` で描画パラメータを上書きできます。
このサンプル音声は 2ch WAV で，左チャンネルが 440Hz，右チャンネルが 660Hz の 3 秒の正弦波です。
wavesurfer.js 本体と Spectrogram plugin は CDN から読み込みます。
remote audio を使う場合は，wavesurfer.js が解析できるよう CORS 設定が必要です。
-->
